...
def eval_genomes_old(genomes, config):
    global nets, ge, inputs, outputs, gen, generationOutputs, minDistance, walls
    ...
    if gen > 0:
        restartGame(balls)
        #print(generationOutputs[gen - 1])

    while running and len(ge) > 0:
        
        #print("Iterazione" + str(len(ge)))

        # + fitness because alive
        #for genome in ge:
        #    genome.fitness += 0.1
        ...
        mesureLines()
            ...
            #for player in players:
                #print("playerrr")
                #print(player.lines.upLine)
                #print(player.lines.downLine)
                #print(player.lines.leftLine)
                #print(player.lines.rightLine)
                #print(player.lines.upRightLine)
                #print(player.lines.upLeftLine)
                #print(player.lines.downRightLine)
                #print(player.lines.downLeftLine)
        ...
        for i, net in enumerate(nets):
            outputArray = net.activate((players[i].lines.upLine, players[i].lines.downLine, players[i].lines.leftLine, players[i].lines.rightLine, players[i].lines.upRightLine, players[i].lines.upLeftLine, players[i].lines.downRightLine, players[i].lines.downLeftLine, players[i].x, players[i].y, end.x, end.y))
        ...
        for i, genome in enumerate(ge):
            ...
            if(players[i].lines.upLine == 0 and generationOutputs[gen][i][len(generationOutputs[gen][i]) - 1][0] == 'up'):
                genome.fitness -= 0.5
                #generationOutputs[gen][i][len(generationOutputs[gen][i]) - 1][0] = ''
            ...
            if(players[i].lines.rightLine == 0 and generationOutputs[gen][i][len(generationOutputs[gen][i]) - 1][1] == 'right'):
                genome.fitness -= 0.5
                #generationOutputs[gen][i][len(generationOutputs[gen][i]) - 1][1] = ''
            ...
            if(generationOutputs[gen][i][len(generationOutputs[gen][i]) - 1][0] == 'up'): # little bit sketchy
                ...
                #if (not players[i].moveDown(walls)):
                #    genome.fitness -= 0.5
                #    generationOutputs[gen][i][len(generationOutputs[gen][i]) - 1][0] = ''
            else:
            ...
            if(generationOutputs[gen][i][len(generationOutputs[gen][i]) - 1][1]  == 'right'):
                ...
                #if (not players[i].moveRight(walls)):
                #    genome.fitness -= 0.5
                #    generationOutputs[gen][i][len(generationOutputs[gen][i]) - 1][1] = ''
                ...
                #if (not players[i].moveLeft(walls)):
                #    genome.fitness -= 0.5
                #    generationOutputs[gen][i][len(generationOutputs[gen][i]) - 1][1] = ''
            else:
                generationOutputs[gen][i][len(generationOutputs[gen][i]) - 1][1] = players[i].x

            #players[i].move(walls)
            players[i].addToHistory()

            #print(str(i) + " - " + str(players[i].getNowRow()) + " - " + str(players[i].getNowColumn()))

            #have to set all the players to false in all direction
            #players[i].up = False
            #players[i].down = False
            #players[i].right = False
            #players[i].left = False
        ...
        for indexCollision in collisions:

            ge[indexCollision].fitness -= 2
            nets.pop(indexCollision)
            ge.pop(indexCollision)
            players.pop(indexCollision)

            
        #Remove fitness if the player is stall in a single place for too many time
        toRemove = []
        for i, player in enumerate(players):
            if(player.haveToRemove(cicleToRemove)):
                toRemove.append(i)

        toRemove.sort(reverse=True)

        for index in toRemove:
            #print("Rimosso da lentezza" + str(index) + " - " + str(len(ge)))
            #ge[index].fitness -= 1
            nets.pop(index)
            ge.pop(index)
            players.pop(index)
        ...
        indexMin = -1
        endPosition = [None, None]

        for i, player in enumerate(players):

            xDistance = abs(player.x - end.x)
            yDistance = abs(player.y - end.y) 
            ...
            #    print(end.x)
            #    print(end.y)
            #    print("x distance: " + str(xDistance))
            #    print("y distance: " + str(yDistance))
            ...
            #There is ne big problem, if between the player and the win there is a wall
...
def checkCollisionAI():
    ...
    for index, player in enumerate(players):
        ...
        #I should add the win to the collision message
